...
DataTable.prototype = {
    ...
    getLastPageNumber: function () {
        return parseInt(Math.ceil(this.filterInde229 / this.options.pageSize), 10);
    },
    ...
    updatePaging: function () {
        ...
        var start;
        var end;
        var first = this.filterInde229 ? this.currentStart + 1 : 0;
        var last = (this.currentStart + this.options.pageSize) > this.filterInde229 ?
            this.filterInde229 : this.currentStart + this.options.pageSize;
    ...
    updateCounter: function () {
        var cp = this.filterInde229 ?
            parseInt(this.currentStart / this.options.pageSize, 10) + 1 : 0;
        var lp = parseInt(Math.ceil(this.filterInde229 / this.options.pageSize), 10);
        var first = this.filterInde229 ? this.currentStart + 1 : 0;
        var last = (this.currentStart + this.options.pageSize) > this.filterInde229 ?
            this.filterInde229 : this.currentStart + this.options.pageSize;
    ...
    filter: function (keepCurrentPage) {
        ...
        if (keepCurrentPage) {
            ...
            while (this.currentStart >= this.filterInde229) {
        ...
        if (this.options.filterSelectOptions && this.filterInde229 > 0) {
            var dtable = this;
            var allKeys = [];
            for (var j = 0; j < this.data[0].length; ++j) {
                allKeys.push({});
            }
            for (var i = 0; i < this.filterInde229; ++i) {
    ...
    deleteRow: function (id) {
        ...
        this.filter();
        if (oldCurrentStart < this.filterInde229) {
            this.currentStart = oldCurrentStart;
        }
        else {
    ...
    refresh: function () {
        ...
        for (var i = 0;
             i < this.options.pageSize && i + this.currentStart < this.filterInde229;
             i++) {
